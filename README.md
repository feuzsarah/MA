...
The basic structure of this code (and how to use it) remains the same as in the base code. Additionally, the different negative samples methods for the training procedure can be configured in the config-file neg_samples.yaml. This version has only been tested for algorithm = 'threshold' and loss = 'entropic'. In contrast to the base code, one also has to assign an experiment number to each experiment (in the threshold.yaml and the neg_samples.yaml) for training and evaluation.
