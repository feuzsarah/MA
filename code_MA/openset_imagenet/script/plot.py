...
def load_scores(args, cfg, cfg_ns):
        ...
        for loss in args.losses:
            for algorithm in args.algorithms:
                for methods in ['real','noise']:
                    ...
                    score_file = output_directory / f"{loss}_{alg}_test_arr_{suffix}_{methods}.npz"
...
#        fig = pyplot.figure(figsize=(3*A+1, 2*P))
        ...
        for l, loss in enumerate(args.losses):
            ...
            for a, algorithm in enumerate(algorithms):
                ...
                #ax = axs[l,a]
                ax = axs[0]
                print(scores[2]['entropic']['threshold'])
                if scores[2]['entropic']['threshold'] is not None:
...
def ccr_table(args, scores, gt):
    ...
    for protocol in args.protocols:
        ...
        for algorithm in args.algorithms:
            for loss in args.losses:
                ccrs = openset_imagenet.util.ccr_at_fpr(gt[2], scores[2]['entropic']['threshold'], args.fpr_thresholds, unk_label=-1)
                results_n[2]['entropic'] = ccrs
                ...
                max_by_alg_n['threshold'] = nonemax(max_by_alg_n[algorithm], ccrs)
                max_by_loss_n['entropic'] = nonemax(max_by_loss_n[loss], ccrs)

                ccrs = openset_imagenet.util.ccr_at_fpr(gt[2], scores[2]['entropic']['threshold'], args.fpr_thresholds, unk_label=-2)
                results_u['threshold']['entropic'] = ccrs
                max_total_u = nonemax(max_total_u, ccrs)
                max_by_alg_u['threshold'] = nonemax(max_by_alg_u['threshold'], ccrs)
                max_by_loss_u['entropic'] = nonemax(max_by_loss_u['entropic'], ccrs)
...
#            f.write("\\bf Algorithm & \\bf Loss & ")
            ...
            f.write("\\\\\\hline\\hline\n")
            for algorithm in args.algorithms:
                f.write(f"\\multirow{{{len(args.losses)}}}{{*}}{{{NAMES['threshold']}}}")
                ...
                f.write(f" & {NAMES['entropic']}")
                print(results_u)
                for i, v in enumerate(results_n[2]['entropic'][:-1]):
                    if v is None: f.write(" &")
                    elif v == max_total_n[i]: f.write(f" & \\textcolor{{blue}}{{\\bf {v:.4f}}}")
                    elif v == max_by_alg_n['threshold'][i]: f.write(f" & \\it {v:.4f}")
                    elif v == max_by_loss_n['entropic'][i]: f.write(f" & \\underline{{{v:.4f}}}")
                    else: f.write(f" & {v:.4f}")
                for i, v in enumerate(results_u['threshold']['entropic']):
                    if v is None: f.write(" &")
                    elif v == max_total_u[i]: f.write(f" & \\textcolor{{blue}}{{\\bf {v:.4f}}}")
                    elif v == max_by_alg_u['threshold'][i]: f.write(f" & \\it {v:.4f}")
                    elif v == max_by_loss_u['entropic'][i]: f.write(f" & \\underline{{{v:.4f}}}")
                    else: f.write(f" & {v:.4f}")
                f.write("\\\\\n")
                f.write("\\hline\n")
...
def main(command_line_arguments = None):
    ...
    config_neg = openset_imagenet.util.load_yaml('config/neg_samples.yaml')
...
#        level="INFO")
    ...
    scores, ground_truths = load_scores(args, cfg, config_neg)
